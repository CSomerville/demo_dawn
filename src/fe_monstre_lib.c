...
static void init_monstre_data(FEMonstreData *dat) {
...
static void copy_entity_by_index(DDTwine *tw, FEMonstreData *dat,
		unsigned int index) {
	dd_twine_copy(tw, &dat->entities.elems[index]);
}

static int pick_random_entity(FEMonstreData *dat) {
	int r = rand() % dat->entities.size;
	return r;
}

static void filter_relations_by_subject(DDArrInt *indices,
		int subject_index, FEMonstreData *dat) {
	int i;
...
static void filter_content_by_relation_type(DDArrInt *indices,
		int relation_type_index, FEMonstreData *dat) {
	int i;
